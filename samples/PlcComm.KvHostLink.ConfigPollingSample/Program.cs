    ...
    bool DryRun)
{
    ...
    {
        Console.WriteLine("Run read-only KEYENCE KV Host Link polling from a JSON config file.");
        Console.WriteLine("Usage:");
        Console.WriteLine("  dotnet run --project samples/PlcComm.KvHostLink.ConfigPollingSample -- --config samples/PlcComm.KvHostLink.ConfigPollingSample/config_polling.example.json --dry-run");
    ...
    TimeSpan MaxBackoff)
{
    public static PollingPlan Load(ConfigPollingOptions options)
    {
        ...
        foreach (var plcElement in plcsElement.EnumerateArray())
        {
            ...
            var endpoint = new PlcEndpoint(
                name,
                GetRequiredString(plc, "host", $"plcs[{index}].host"),
                profileText,
        ...
        return new PollingPlan(
            endpoints,
            tagsByPlc,
            ResolveOutputPath(options.ConfigPath, GetOptionalString(output, "csv")),
            cycles,
            options.InitialBackoffOverride ?? TimeSpan.FromSeconds(GetPositiveDouble(root, "initial_backoff", 1)),
            options.MaxBackoffOverride ?? TimeSpan.FromSeconds(GetPositiveDouble(root, "max_backoff", 30)));
    }
    ...
    private static List<TagSpec> ParseTags(JsonElement plc, string plcName)
    {
        ...
        {
            tags.Add(item.ValueKind == JsonValueKind.String
                ? OperationalCommon.ParseTagSpec(item.GetString() ?? string.Empty)
                : new TagSpec(GetRequiredString(item, "name", "tag.name"), GetRequiredString(item, "address", "tag.address")));
        }
        if (tags.Count == 0)
    ...
    private static double GetPositiveDouble(JsonElement element, string propertyName, double defaultValue)
    {
        if (element.ValueKind == JsonValueKind.Undefined || !element.TryGetProperty(propertyName, out var value))
            return defaultValue;
        if (!value.TryGetDouble(out var parsed) || parsed <= 0)
...
internal sealed class CsvSnapshotWriter(string path) : IDisposable
{
    ...
    public async Task WriteSnapshotAsync(PlcEndpoint endpoint, IReadOnlyDictionary<string, object> snapshot, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (var item in snapshot)
            {
                ...
                await writer.WriteLineAsync(string.Join(",", CsvEscape(timestamp), CsvEscape(endpoint.Name), CsvEscape(item.Key), CsvEscape(OperationalCommon.FormatValue(item.Value)))).ConfigureAwait(false);
            }
        }
        finally
        {
