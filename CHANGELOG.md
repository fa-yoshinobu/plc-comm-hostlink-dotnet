...
### Changed
- [Library docs] Documented `DM100:COMMENT` named reads in the public .NET XML documentation.
...
- Updated samples and examples to pass the PLC profile argument explicitly.
...
### Added
- Added `KvTimerCounterValue` plus `ReadTimerCounterAsync`,
  `ReadTimerAsync`, and `ReadCounterAsync` helpers for full `T` / `C`
  composite values.
