...
This repository publishes `PlcComm.KvHostLink`. NuGet publishing is
intentionally manual, but GitHub Releases must stay synchronized with the
manually published NuGet version.

## Required Artifacts

Every GitHub Release must include these three assets:

- `PlcComm.KvHostLink.<version>.nupkg`
- `PlcComm.KvHostLink.<version>.snupkg`
- `PlcComm.KvHostLink.<version>.dll.zip`
...
- Keep `<Version>` in `src\PlcComm.KvHostLink\PlcComm.KvHostLink.csproj` equal
...
5. Build release artifacts from the tag:
   ...
   dotnet restore PlcComm.KvHostLink.sln
   dotnet build PlcComm.KvHostLink.sln -c Release --no-restore
   dotnet test PlcComm.KvHostLink.sln -c Release --no-build
   dotnet pack src\PlcComm.KvHostLink\PlcComm.KvHostLink.csproj -c Release --no-build --output out
   Compress-Archive -Path src\PlcComm.KvHostLink\bin\Release\net9.0\* -DestinationPath out\PlcComm.KvHostLink.X.Y.Z.dll.zip -Force
   git switch main
   ```
...
```powershell
gh release view vX.Y.Z --repo fa-yoshinobu/plc-comm-hostlink-dotnet --json tagName,name,assets,url
gh release list --repo fa-yoshinobu/plc-comm-hostlink-dotnet --limit 3
```
...
- NuGet shows the same `PlcComm.KvHostLink` version;
