...
This document is the release checklist for `PlcComm.KvHostLink`.
...
1. Update `<Version>` in `src/PlcComm.KvHostLink/PlcComm.KvHostLink.csproj`.
...
3. Cross-library parity
   - Public API surface matches the Host Link Python library equivalents.
...
Confirm package metadata in `src/PlcComm.KvHostLink/PlcComm.KvHostLink.csproj`:
