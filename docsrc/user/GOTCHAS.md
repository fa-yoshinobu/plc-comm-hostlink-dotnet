...
| Bit-in-word write | `WriteBitInWordAsync` or `WriteBitInExpansionUnitBufferAsync` can overwrite a concurrent change to another bit in the same word. | These explicit helpers are two-request, non-PLC-atomic read-modify-writes. Use PLC-side coordination or exclusive whole-word ownership, and never automatically retry an outcome-unknown result. |
